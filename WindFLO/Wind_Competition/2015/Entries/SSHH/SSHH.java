...
public class SSHH {
	...
	// return the number of low level heuristics
	...
	// ensure that the turbine can be inserted at location "index" without violating any constraint
	...
	// mutational low level heuristic
	...
	private boolean evaluate() {
		...
		if (verbose) {
			System.out.println(ind+"\t"+obj+"\t"+bestObj+"\t"+wfle.getEnergyCost() + "\t" + best_shift + "\t" + best_step_x + "\t" + best_step_y + "\t" + getNumberOfEvaluation);
		}
		return is_best_solution_improved;
	}
	
	/*
	private boolean evaluate_2() {
		//check all zeros;
		boolean allZeros = true;
		for (int i=0; i<grid.size(); i++) {
			if (solution[i]) {
	...
	*/
	...
	public void run() {
		...
		if (verbose) {
			...
			for (int i=0; i<getNumberOfLowLevelHeuristics(); ++i) {
				System.out.println("LLH"+i+"\t"+utilisation[i]);
			}
			int[][] matrixAS = new int [getNumberOfLowLevelHeuristics()][2];
			int[][] matrixP = new int [getNumberOfLowLevelHeuristics()][3];
			int[][] matrixLLH = new int [getNumberOfLowLevelHeuristics()][getNumberOfLowLevelHeuristics()];
			for (int i=0; i<getNumberOfLowLevelHeuristics(); ++i) {
				for (int j=0; j<2; ++j) {
					matrixAS[i][j] = 0;
				}
				...
				for (int j=0; j<getNumberOfLowLevelHeuristics(); ++j) {
					matrixLLH[i][j] = 0;
				}
			}
			for (int i=0; i<getNumberOfLowLevelHeuristics(); ++i) {
				for (int j=0; j<heuristic[i].no_added_move_acceptances;++j) {
					matrixAS[i][heuristic[i].sequenceBasedAcceptanceStrategySelection[j]]++;
				}
			}
			for (int i=0; i<getNumberOfLowLevelHeuristics(); ++i) {
				for (int j=0; j<heuristic[i].no_added_parameters;++j) {
					matrixP[i][heuristic[i].parameterSelection[j]]++;
				}
			}
			for (int i=0; i<getNumberOfLowLevelHeuristics(); ++i) {
				for (int j=0; j<heuristic[i].no_added_next_heuristics;++j) {
					matrixLLH[i][heuristic[i].nextHeuristicSelection[j]]++;
				}
			}
			System.out.println("NextLLHMatrix");
			for (int i=0; i<getNumberOfLowLevelHeuristics(); ++i) {
				System.out.print("\tLLH"+i);
			}
			System.out.println();
			for (int i=0; i<getNumberOfLowLevelHeuristics(); ++i) {
				System.out.print("LLH"+i+"\t");
				for (int j=0; j<getNumberOfLowLevelHeuristics();++j) {
					System.out.print(matrixLLH[i][j]+"\t");
				}
				System.out.println();
			}
			...
			System.out.println("SolutionsRecord");
			System.out.println("SolutionsSize\t"+solutionsRecord.size()+"\tnoSolutionsAlreadyBeingEvaluated\t"+noSolutionsAlreadyEvaluated);
		}
		
		/*if (verbose) System.out.println("RCTIFY");
		for (int i=0; i<getNumberOfRows(); i++) {
			if (getNumberOfEvaluation >= 2000) break;
			for (int j=0; j<getNumberOfCols(); j++) {
				if (getNumberOfEvaluation >= 2000) break;
				if (indexInSolution[i][j] != -1) {
					if (solution[indexInSolution[i][j]] == false) {
						solution[indexInSolution[i][j]] = true;
						FixPosition(indexInSolution[i][j]);
						if (solution[indexInSolution[i][j]]) evaluate_2();
					}
				}
			}
		}
		...
		if (getNumberOfEvaluation < 2000) evaluate_2();*/
	}
}