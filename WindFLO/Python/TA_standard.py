...
def run_ta_standard(grid,java_evaluator,best_pop=None,best_fit=1,t = 0.1,alpha = 0.95, n_final = 100000,cycle_limit=1000):
    ...
    while n < n_final:
        ...
        if best_fits[-1] < best_fit + t:
            with open('test_TA_standard_right_layout_1000.csv','w+') as f:
                a = csv.writer(f)
                a.writerows(layout)
        ...
        with open('test_TA_standard_right_fits_1000.csv','a+') as f:
