...
def run_ta_standard_right1(grid,java_evaluator,best_pop=None,best_fit=1,t = 0.1,alpha = 0.95, n_final = 100000,cycle_limit=1000):
    ...
    if type(best_pop) == type(None):
        ...
        bins = np.random.rand(max_turbs) > 0.1
        best_pop[:] = bins
    ...
    while n < n_final:
        ...
        if best_fits[-1] < best_fits[-2] + t:
            with open('test_TA_standard_right1_layout_100000.csv','w+') as f:
                a = csv.writer(f)
                a.writerows(layout)
        ...
        with open('test_TA_standard_right1_fits_100000.csv','a+') as f:
